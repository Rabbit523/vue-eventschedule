...
.v-select .dropdown-toggle {
  border: 1px solid #14546d !important;
  background-color: #0c3241 !important;
}
.v-select input[type="search"]:focus {
  box-shadow: 0 0 8px rgba(0, 168, 232, 0.6) !important;
}
...
.v-select .dropdown-menu {
  ...
  opacity: 1 !important;
  background-color: rgba(0, 23, 31, 1) !important;
  ...
  <div :dir="dir" class="dropdown v-select" :class="dropdownClasses">
    <div ref="toggle" @mousedown.prevent="toggleDropdown" :class="['dropdown-toggle', 'clearfix']">
      ...
      <span class="selected-tag" v-for="option in valueAsArray" v-bind:key="option.index">
				...
				<button :disabled="disabled" @click="edit(option)" type="button" class="edit" aria-label="Edit option">         
					<slot name="selected-option" v-bind="option">
          {{ getOptionLabel(option) }}
        	</slot>
        </button>
        <button :disabled="disabled" @click="deselect(option)" type="button" class="close" aria-label="Remove option">
    ...
    <transition :name="transition">
      <ul ref="dropdownMenu" v-if="dropdownOpen" class="dropdown-menu" :style="{ 'max-height': maxHeight }">
        <li v-for="(option, index) in filteredOptions" v-bind:key="index" :class="{ active: isOptionSelected(option), highlight: index === typeAheadPointer }" @mouseover="typeAheadPointer = index">
          <a @mousedown.prevent="select(option, $event)">
          <slot name="option" v-bind="option">
            {{ getOptionLabel(option) }}
          </slot>
          </a>
        </li>
        <li :class="{highlight:true}" @mouseover="typeAheadPointer = filteredOptions.length">
          <a @mousedown.prevent="createnew()" >
          <slot name="options">New Venue</slot>
          </a>
        </li>
...
export default {
  ...
  methods: {
    ...mapActions("events", ["setVenueDisplay", "setSelectedVenue", "setNewVenue", "setVenuePopulate"]),
    /**
    ...
    select(option, event) {
      ...
      if (this.clicks === 1) {
        ...
        option.id = "selectVenue";
        this.setSelectedVenue(option);
    ...
    createnew() {     
      ...
      this.setNewVenue({ name: "", id: "newVenue" });
      this.setVenueDisplay(true);
      this.setVenuePopulate(this.search);
      this.search = "";
    },
    /**
